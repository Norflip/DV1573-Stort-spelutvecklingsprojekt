	...
	float2 uv : SV_DomainLocation)
{
	...
	float disp = 0.008f * uv.x;
	float dispT = 0.05f * uv.x * (noiseSample + 2.5f * abs(sin((time * 1) + noiseSample)));

	//float disp = /*grassDisplacement*/ 0.008f * pow(uv.x, 1.0); //grassDisplacement == Length
