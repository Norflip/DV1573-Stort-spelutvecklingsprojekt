...
ParticleSystem::ParticleSystem(const ParticleSystem& other)
{
	...
	this->bufferPerObjectParticles = other.bufferPerObjectParticles;
...
{	
	...
	particlesPosition.y = object->GetTransform().GetPosition().m128_f32[1] + 0.4f;
...
void ParticleSystem::SetDifference(float x, float y, float z)
{
...
void ParticleSystem::UpdateBuffers(ID3D11DeviceContext* context)
{
	...
	*/
	...
	HRESULT result = context->Map(vertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);	// maybe D3D11_MAP_WRITE_NO_OVERWRITE
	assert(SUCCEEDED(result));	
